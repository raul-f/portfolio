...
function FilterInfo(props: FilterInfoProps) {
  ...
  const active = props.active as keyof typeof descriptions;
  const activeOptions = descriptions[active];
  const language = props.language as keyof typeof activeOptions;

  return (
    <div className="filter-info">
      <p className="filter-info-text">{descriptions[active][language]}</p>
    </div>
  );
}
