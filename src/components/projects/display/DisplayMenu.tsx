...
function DisplayMenu(props: DisplayMenuProps) {
  const sets = {
    "personal-picks": {
    ...
    "full-stack-web-apps": {
      ...
      setOptions: [
        ...
        {
          name: { "en-US": "GitHub Stars", "pt-BR": "GitHub Stars" },
          id: "github-stars",
          hash: "bc0ff06f466126cc64fe8b01ae496fe3",
        },
      ],
  ...
  const set = props.set as keyof typeof sets;
  const language = props.language as "pt-BR" | "en-US";

  const setOptions = sets[set].setOptions.map((value) => (
    <button
      key={value.hash}
      id={value.id}
      ...
      className={
        props.selected === value.id ? "display-option sel" : "display-option"
      }
    >
      {value.name[language]}
    </button>
  ...
  return (
    <div className="display-menu-box">
      <div className="display-menu">
        <h3 className="menu-title">{sets[set].setName[language]}</h3>
